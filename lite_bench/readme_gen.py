...
def generate(results: dict, config: Config, chart_paths: list[str]) -> str:
    ...
    _a("| Ollama (local) | *(none needed)* | [ollama.com](https://ollama.com) |")
    _a("| HuggingFace (GPQA) | `HF_TOKEN` | [huggingface.co/settings/tokens](https://huggingface.co/settings/tokens) |")
    _a("")
    _a("> **Note:** GPQA is a gated dataset. You must agree to its terms at")
    _a("> [huggingface.co/datasets/Idavidrein/gpqa](https://huggingface.co/datasets/Idavidrein/gpqa)")
    _a("> and set `HF_TOKEN` to download it. All other datasets are public.")
    _a("")
    _a("### Commands")
    _a("")
    _a("```bash")
